...
PROVIDER_MODEL = 'bycn_saml.provider'
...
class SAMLLogin(Home):
    ...
    def list_providerss(self):
        providers = request.env[PROVIDER_MODEL].sudo().search_read([])
    ...
    def web_login(self, *args, **kw):
        ...
        providers = self.list_providerss()

        response = super().web_login(*args, **kw)
        if response.is_qweb:
            error = request.params.get('saml_error')
            errors = {
                'no-signup': _('Sign up is not allowed on this database.'),
                'access-denied': _('Access Denied.'),
                'expired': _('You do not have access to this database or your '
            ...
            response.qcontext['providerss'] = providers
            if error:
...
class AuthSAMLController(http.Controller):
    ...
    @http.route('/metadata', type='http', auth='none')
    def get_metadata(self, pid=False):
        # 1- If no pid, we will take the first provider in database
        domain = []
        if pid:
            domain.append(
                ('id', '=', int(pid))
        ...
        # 2- Fetching provider from database
        provider_id = request.env[
            PROVIDER_MODEL
        ].sudo().search(
            domain,
            limit=1
        ...
        # 3- Getting XML content from provider metadata field
        return Response(
            provider_id[0].sp_metadata,
    ...
    def signin(self, req, **kw):
        ...
        redirect = self._get_safe_redirect(
            redirect=state.get('r', False)
        )
        provider = state['p']
        dbname = state['d']
        context = state.get('c', {})
        ...
        with registry.cursor() as cr:
            try:
                ...
                credentials = env['res.users'].sudo().auth_saml(
                    provider, saml_response, redirect
                )
                after_login_url = credentials[3]
                # Setting credentials back to old format
                credentials = credentials[0], credentials[1], credentials[2]
                action = state.get('a')
                menu = state.get('m')
                url = '/'
                if action:
            ...
            except odoo.exceptions.AccessDenied as acc:
                # saml credentials not valid,
                # user could be on a temporary session
                _logger.exception(acc)
    ...
    def _get_safe_redirect(self, redirect):
        redirect_to_web = '%sweb' % (
            request.httprequest.url_root
        )
        if werkzeug.urls.url_quote_plus(redirect_to_web) == redirect:
            return False
        return redirect
