...
class BoardStock(models.AbstractModel):
    _name = 'board.stock'
    _description = "Board Stock"
    _auto = False
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(BoardStock, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)

        custom_view = self.env['ir.ui.view.custom'].search([('user_id', '=', self.env.uid), ('ref_id', '=', view_id), ('board_stock', '=', True)], limit=1)
        if custom_view:
